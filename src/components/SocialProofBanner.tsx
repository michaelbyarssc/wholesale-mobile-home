...
export const SocialProofBanner = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const {
          data: settingsData
        } = await supabase.from("social_proof_settings").select("*").single();
        if (settingsData) {
          ...
          if (settingsData.show_recent_purchases) {
            const {
              data: purchasesData
            } = await supabase.from("recent_purchases").select("*").eq("active", true).order("purchase_date", {
              ascending: false
  ...
  if (variant === "header") {
    return <div className={`bg-gradient-to-r from-green-500 to-green-600 text-white py-2 px-4 ${className}`}>
        <div className="max-w-7xl mx-auto flex items-center justify-center text-center">
          <div className="flex items-center gap-4 text-sm font-medium">
            {settings.show_customer_count && <div className="flex items-center gap-1">
                <Users className="w-4 h-4" />
                <span>{settings.customer_count.toLocaleString()}+ Happy Customers</span>
              </div>}
            {settings.show_homes_sold && <div className="flex items-center gap-1">
                <Home className="w-4 h-4" />
                <span>{settings.homes_sold_count.toLocaleString()}+ Homes Sold</span>
              </div>}
            {settings.show_recent_purchases && currentPurchase && <div className="flex items-center gap-1">
  ...
  if (variant === "embedded") {
    return <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 ${className}`}>
        {settings.show_customer_count && <Card className="text-center">
        ...
        {settings.show_homes_sold && <Card className="text-center">
            <CardContent className="pt-6">
              <div className="flex items-center justify-center w-12 h-12 bg-primary/10 rounded-full mx-auto mb-4">
              ...
              <p className="text-muted-foreground">Homes Delivered Last Month</p>
            </CardContent>
          </Card>}
  ...
  return <div className={`fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-96 z-50 ${className}`}>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 mb-2">
                ...
                {currentPurchase.purchase_amount && <span className="text-green-600 font-semibold ml-2">
                    ${currentPurchase.purchase_amount.toLocaleString()}
                  </span>}
              </p>
              
              <p className="text-xs text-gray-500">
                {formatDistanceToNow(new Date(currentPurchase.purchase_date), {
                addSuffix: true
              })}
              </p>
            </div>
            
            <button onClick={() => setIsVisible(false)} className="text-gray-400 hover:text-gray-600 text-lg leading-none">
              ×
            </button>
          </div>
        </CardContent>
