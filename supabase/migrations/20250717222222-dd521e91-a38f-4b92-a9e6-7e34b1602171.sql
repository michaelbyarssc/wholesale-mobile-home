-- Create cron schema and extension
...
-- Create materialized view for analytics overview that refreshes every hour
...
CREATE MATERIALIZED VIEW IF NOT EXISTS analytics_popular_pages_mv AS
SELECT 
  page_path,
  page_title,
  COUNT(*) as views,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS analytics_mobile_homes_mv AS
WITH home_stats AS (
  SELECT
    mhv.mobile_home_id,
    mh.model,
...
BEGIN
  ...
  REFRESH MATERIALIZED VIEW CONCURRENTLY analytics_popular_pages_mv;
  REFRESH MATERIALIZED VIEW CONCURRENTLY analytics_mobile_homes_mv;
END;
$$;
