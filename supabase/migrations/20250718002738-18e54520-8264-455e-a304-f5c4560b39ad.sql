-- Fix the update_invoice_balance function to properly handle order creation with total_value
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    IF new_balance <= 0 AND NOT EXISTS (
      ...
      -- Create order first if it doesn't exist (required for delivery)
      IF NOT EXISTS (
        SELECT 1 FROM orders 
        WHERE customer_email = invoice_record.customer_email 
        AND user_id = invoice_record.user_id
        AND mobile_home_id = invoice_record.mobile_home_id
      ) THEN
        INSERT INTO public.orders (
          ...
          default_company_id, invoice_record.customer_name,
          invoice_record.customer_email, invoice_record.customer_phone,
          invoice_record.user_id, invoice_record.mobile_home_id,
          invoice_record.estimate_id, invoice_record.total_amount,
          'Net 30', 'confirmed', COALESCE(NEW.created_by, auth.uid())
