-- Fix the create_tracking_session_for_delivery trigger to use proper total_value
...
BEGIN
  ...
  -- Get estimate total amount
  SELECT total_amount INTO estimate_total
  FROM estimates
  ...
  IF new_order_id IS NULL THEN
    INSERT INTO orders (
      ...
      NEW.customer_phone,
      NEW.mobile_home_id,
      COALESCE(estimate_total, 0), -- Use estimate total or 0 as fallback
      'confirmed',
