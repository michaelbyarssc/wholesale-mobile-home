-- Fix the record_invoice_payment_optimized function to use correct column name
...
BEGIN
  ...
  IF new_balance = 0 AND NOT EXISTS (SELECT 1 FROM deliveries WHERE invoice_id = p_invoice_id) THEN
    ...
    -- Create delivery record with correct column name (mobile_home_type, not home_type)
