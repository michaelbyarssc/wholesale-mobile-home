-- Fix the record_invoice_payment_optimized function to include pickup_address
...
BEGIN
  ...
  IF new_balance = 0 AND NOT EXISTS (SELECT 1 FROM deliveries WHERE invoice_id = p_invoice_id) THEN
    ...
    -- Get mobile home details and factory address for pickup
    SELECT mh.width_feet, f.street_address || ', ' || f.city || ', ' || f.state || ' ' || f.zip_code as factory_address
    INTO home_record
    ...
    -- Set factory address or default
    factory_address := COALESCE(home_record.factory_address, 'Factory Address TBD');
    ...
    -- Create delivery record with required pickup_address
