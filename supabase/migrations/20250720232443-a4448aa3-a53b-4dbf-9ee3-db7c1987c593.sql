-- Fixed migration for timezone-aware date storage
...
UPDATE delivery_status_history SET
  created_at_tz = convert_utc_to_tz_string(created_at, (SELECT delivery_address FROM deliveries WHERE deliveries.id = delivery_status_history.delivery_id));
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'delivery_status_history' THEN
    NEW.created_at_tz := convert_utc_to_tz_string(COALESCE(NEW.created_at, NOW()), 
      (SELECT delivery_address FROM deliveries WHERE id = NEW.delivery_id));
  END IF;
