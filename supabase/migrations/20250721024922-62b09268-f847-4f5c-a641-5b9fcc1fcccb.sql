-- Fix delivery transaction number assignment to use next sequence number
...
BEGIN
  ...
  CASE record_type
    ...
    WHEN 'deliveries' THEN
      IF NEW.transaction_number IS NULL THEN
        -- For deliveries, always generate a new transaction number (don't reuse from invoice/estimate)
