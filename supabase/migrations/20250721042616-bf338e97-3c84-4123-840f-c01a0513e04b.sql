-- ADD NEEDS_SCHEDULED STATUS TO DELIVERY_STATUS ENUM IF IT DOESN'T EXIST

-- First, check if needs_scheduled already exists, if not add it
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'needs_scheduled' AND enumtypid = 'delivery_status'::regtype) THEN
        ALTER TYPE delivery_status ADD VALUE 'needs_scheduled';
    END IF;
END
$$;

-- Update the function to use needs_scheduled status
    ...
    IF new_balance = 0 THEN
      ...
      IF NOT EXISTS(SELECT 1 FROM public.deliveries WHERE invoice_id = NEW.invoice_id) THEN
        ...
        SELECT 
          ...
          END,
          'needs_scheduled'::delivery_status, -- PAYMENT IS COMPLETE, DELIVERY NEEDS TO BE SCHEDULED
          i.total_amount,
