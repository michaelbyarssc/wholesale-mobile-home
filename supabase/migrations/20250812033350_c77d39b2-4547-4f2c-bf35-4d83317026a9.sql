-- Secure newsletter_subscribers table RLS and policies (retry with EXECUTE strings)
DO $$
DECLARE
  pol RECORD;
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admins can view newsletter subscribers" ON public.newsletter_subscribers FOR SELECT USING (is_admin(auth.uid()));';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Anyone can subscribe to newsletter" ON public.newsletter_subscribers FOR INSERT WITH CHECK (true);';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Only admins can update newsletter subscribers" ON public.newsletter_subscribers FOR UPDATE USING (is_admin(auth.uid())) WITH CHECK (is_admin(auth.uid()));';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Only admins can delete newsletter subscribers" ON public.newsletter_subscribers FOR DELETE USING (is_admin(auth.uid()));';
  END IF;
END
$$;