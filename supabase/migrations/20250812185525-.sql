-- Create secure RPC to schedule factory pickup, assign drivers, and log status
...
declare
  d record;
  prev_status text;
  new_status text := 'factory_pickup_scheduled';
...
begin
  ...
  prev_status := d.status::text;

  -- Update delivery core fields
  update deliveries
  set
    status = new_status,
    scheduled_pickup_date = coalesce(p_scheduled_pickup, scheduled_pickup_date),
    scheduled_delivery_date = coalesce(p_scheduled_delivery, scheduled_delivery_date),
  ...
  if p_driver_ids is not null then
    foreach drv in array p_driver_ids loop
      insert into delivery_assignments (
        ...
        did, drv, now(), auth.uid(), true, 'driver'
      );
    end loop;
  end if;
